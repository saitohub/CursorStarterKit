...
執筆フェーズでは **必ず `.claude/skills/note-word/references/style-guide.md` を Read** し、
...
| 3 | WebSearch + WebFetch（Claude Code 標準ツール） | 上記スキルが無い場合のフォールバック |

> 配布先の Claude Code 環境に `/research-free` が無い可能性もあるため、
...
- `{slug}`: 記事テーマから生成する英小文字スラッグ（例 `claude-note-automation`）
...
- `image_mode` の各値は `nanobanana` / `claude-code` / `placeholder` / `manual` のいずれか
...
1. `.claude/skills/note-word/references/style-guide.md` を Read
...
`.claude/skills/nanobanana-deji/` フォルダは同梱しているが、**自動フローでは呼ばない**。
...
実行前に **必ず** Read：
1. `.claude/skills/note-word/references/figure-patterns.md` — 共通設計原則・禁止事項
2. B1ルート利用時：`.claude/skills/note-word/templates/html/*.html` の対象テンプレ
3. B2ルート利用時：`.claude/skills/note-word/templates/figure_template_*.py` の対象テンプレ
...
cp ".claude/skills/note-word/templates/html/thumbnail_light.html" \
   ".claude/skills/note-word/templates/html/_gen_thumbnail.html"
...
python .claude/skills/note-word/scripts/html_to_png.py \
  --html ".claude/skills/note-word/templates/html/_gen_thumbnail.html" \
...
rm ".claude/skills/note-word/templates/html/_gen_thumbnail.html"
```
...
cp ".claude/skills/note-word/templates/figure_template_thumbnail.py" \
...
サムネイル設計の参考は `.claude/skills/note-word/THUMBNAIL_GUIDE.md`。
...
```bash
python .claude/skills/note-word/scripts/auto_convert_tables.py \
...
```bash
python .claude/skills/note-word/scripts/count_chars.py \
  --input "output/articles/{YYYYMMDD}_{slug}/article.md" \
...
```bash
python .claude/skills/note-word/scripts/build_docx.py \
  --input "output/articles/{YYYYMMDD}_{slug}/article.md" \
